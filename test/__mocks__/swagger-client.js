...
const mockSwaggerClient = {
  ...
  mockResolved: function (methodName, returnValue, isFile) {
    ...
    return this.mockFn(methodName).mockImplementation(async () => {
      // call requestInterceptor
      if (mockSwaggerClient.requestInterceptor) {
        mockSwaggerClient.requestInterceptor()
      }
      return val
    })
  },
...
const setAndReturnMock = (params = {}) => {
  mockSwaggerClient.requestInterceptor = params.requestInterceptor
  return mockSwaggerClient
}

module.exports = jest.fn(setAndReturnMock)
